...
const App = () => {
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text('Wallet — ' + t.reportTitle, 14, 15);
    doc.setFontSize(10);
    doc.text(`${t.totalIncome}: ${reportIncome.toLocaleString()} ${currency}`, 14, 25);
    doc.text(`${t.totalExpense}: ${reportExpense.toLocaleString()} ${currency}`, 14, 31);
    doc.text(`${t.totalBalance}: ${(reportIncome - reportExpense).toLocaleString()} ${currency}`, 14, 37);
    autoTable(doc, {
      startY: 44,
            ...
            {chartData.length > 0 && (
              <div style={{ backgroundColor: c.card, padding: '18px', borderRadius: '12px', marginBottom: '16px', border: '1px solid ' + c.border }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px', marginBottom: '14px' }}>
                  <h3 style={{ margin: 0, fontSize: '15px' }}>{t.chartTitle}</h3>
                ...
                <ResponsiveContainer width="100%" height={260}>
                  {chartType === 'pie' ? (
                    <PieChart>
                      <Pie data={chartData} cx="50%" cy="50%" outerRadius={85} dataKey="value" label={({ name, value }) => name + ': ' + value.toLocaleString()}>
                    ...
                    <BarChart data={chartData}>
                      ...
                      <XAxis dataKey="name" stroke={c.text} style={{ fontSize: '10px' }} />
                    ...
                    <LineChart data={chartData}>
                      ...
                      <YAxis stroke={c.text} style={{ fontSize: '10px' }} />
                      <Tooltip contentStyle={{ backgroundColor: c.card, border: '1px solid ' + c.border, color: c.text }} />
                      <Line type="monotone" dataKey="value" stroke={c.saveBtn} strokeWidth={2} dot={{ fill: c.saveBtn, r: 4 }} />
                    </LineChart>
                  )}
                </ResponsiveContainer>
              </div>
